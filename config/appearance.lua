...
Config.exit_behavior = "Close" -- OnCleanExit"
...
Config.keys = {
  -- Eigenes Einfügen, das ^M (CR) komplett eliminiert
  {
    key = "V",
    mods = "CTRL|SHIFT",
    action = require("wezterm").action_callback(function(window, pane)
      -- Holt den aktuellen Text aus der Windows-Zwischenablage
      local clipboard = window:get_clipboard()
      if clipboard then
        -- Ersetzt alle Windows-Zeilenenden (\r\n) durch Linux-Zeilenenden (\n)
        local clean_text = clipboard:gsub("\r\n", "\n"):gsub("\r", "\n")
        -- Sendet den bereinigten Text direkt an das aktive WSL-Fenster
        pane:send_paste(clean_text)
      end
    end),
  },
}
